...
import algorithms.hill_climbing_old_heuristic as hill_climbing_old
import algorithms.hill_climbing_new_heuristic as hill_climbing_new
import algorithms.genetic as genetic
import algorithms.simulated_annealing as simulated_annealing
import algorithms.random as random_algorithm

def init_algorithms():
    hill_climbing_old.ui = ui
    hill_climbing_old.ps = ps

    hill_climbing_new.ui = ui
    hill_climbing_new.ps = ps

    genetic.ui = ui
    genetic.ps = ps

    simulated_annealing.ui = ui
    simulated_annealing.ps = ps

    random_algorithm.ui = ui
    random_algorithm.ps = ps
...
def run_algorithm(option, data):
    if '1' in option:
        ui.print_div('HILL CLIMBING OLD HEURISTIC')
        return hill_climbing_old.main(data[0][0], data[0][1], data[1::])
    elif '2' in option:
        ui.print_div('HILL CLIMBING NEW HEURISTIC')
        return hill_climbing_new.main(data[0][0], data[0][1], data[1::])
    elif '3' in option:
        ui.print_div('SIMULATED ANNEALING')
        return simulated_annealing.main(data[0][0], data[0][1], data[1::])
    elif '4' in option:
        ui.print_div('GENETIC ALGORITHM')
        return genetic.main(data[0][0], data[0][1], data[1::])
    elif '5' in option:
        ui.print_div('RANDOM')
        return random_algorithm.main(data[0][0], data[0][1], data[1::])
    else:
        print('Invalid option')
        choose_algorythm(data)
